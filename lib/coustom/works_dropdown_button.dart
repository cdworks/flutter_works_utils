...
const EdgeInsetsGeometry _kUnalignedMenuMargin = EdgeInsetsDirectional.only(start: 16.0, end: 0);

typedef WorksDropdownButtonBuilder = List<Widget> Function(BuildContext context);
...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  void _handleOnTap() {
    final WorksDropdownMenuItem<T> dropdownMenuItem = widget.route.items[widget.itemIndex].item;
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    final double maxHeight = math.max(0.0,  constraints.maxHeight - buttonRect.top - safeHeight - 44 );
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  double getItemOffset(int index) {
    ...
    return offset-itemHeights.first - 10;
  }
...
//          math.max(buttonBottom, bottomLimit);
...
class _MenuItem<T> extends SingleChildRenderObjectWidget {
  ...
  final WorksDropdownMenuItem<T> item;
...
class WorksDropdownMenuItem<T> extends _DropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const WorksDropdownMenuItem({
    Key key,
    this.onTap,
    this.value,
...
class WorksDropdownButtonHideUnderline extends InheritedWidget {
  /// Creates a [DropdownButtonHideUnderline]. A non-null [child] must
  /// be given.
  const WorksDropdownButtonHideUnderline({
    Key key,
    @required Widget child,
  }) : assert(child != null),
        super(key: key, child: child);
  ...
  static bool at(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<WorksDropdownButtonHideUnderline>() != null;
  }

  @override
  bool updateShouldNotify(WorksDropdownButtonHideUnderline oldWidget) => false;
}
...
class WorksDropdownButton<T> extends StatefulWidget {
  ...
  WorksDropdownButton({
    ...
    // DropdownButtonFormField.
      ...
      items.where((WorksDropdownMenuItem<T> item) {
  ...
  /// be displayed.
  final List<WorksDropdownMenuItem<T>> items;
  ...
  final double paddingHeight; //顶部和底部的高度（一般来说是navigation bar tabbar高度之和）
  ...
  final WorksDropdownButtonBuilder selectedItemBuilder;
...
class _DropdownButtonState<T> extends State<WorksDropdownButton<T>> with WidgetsBindingObserver {
  int _selectedIndex;
  ...
  @override
  void didUpdateWidget(WorksDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
        widget.items.where((WorksDropdownMenuItem<T> item) => item.value == widget.value).length == 1);
    _selectedIndex = null;
  ...
  Widget build(BuildContext context) {
    ...
    if (!WorksDropdownButtonHideUnderline.at(context)) {
...
class WorksDropdownButtonFormField<T> extends FormField<T> {
  ...
  WorksDropdownButtonFormField({
    Key key,
    @required List<WorksDropdownMenuItem<T>> items,
    WorksDropdownButtonBuilder selectedItemBuilder,
    T value,
    Widget hint,
    Widget disabledHint,
    @required this.onChanged,
          ...
          return Focus(
            ...
            child: Builder(builder: (BuildContext context) {
              return InputDecorator(
                ...
                child: WorksDropdownButtonHideUnderline(
                  child: WorksDropdownButton<T>(
                    items: items,
...
class _DropdownButtonFormFieldState<T> extends FormFieldState<T> {
  @override
  WorksDropdownButtonFormField<T> get widget => super.widget as WorksDropdownButtonFormField<T>;
